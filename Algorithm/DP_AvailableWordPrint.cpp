...
//in output[]
void printWordUtil(int number[],int curr_digit,char output[],int n)
{
    ...
    for(i=0;i<strlen(hashTable[number[curr_digit]]);i++)
    {
        ...
        printWordUtil(number,curr_digit+1,output,n);
        if(number[curr_digit]==0 || number[curr_digit]==1)
            return;
    }
}

//A wrapper over printWordUtil(). It creates an output array and
